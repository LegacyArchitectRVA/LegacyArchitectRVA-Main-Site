...
(() => {
  ...
  async function post(path, body, keepalive = false) {
    try {
      const response = await fetch(`${API}/${path}`, {
        method: 'POST', headers: headers(), body: JSON.stringify(body), keepalive
      });
      if (!response.ok) throw new Error(`telemetry-http-${response.status}`);
      return true;
    } catch (error) {
      if (path === 'readiness_events') {
        console.warn('[readiness telemetry]', error.message);
      }
      return false;
    }
  }
  ...
  async function flush(force = false) {
    ...
    if (!force && queue.length < MAX_BATCH) return;
    flushing = true;
  ...
  function event(name, extra = {}) {
    ...
    queue.push({
      id: uuid(), session_id: sessionId, visitor_id: visitorId,
      event_name: name,
  ...
  function sessionPayload() {
    ...
    return {
      ...
      landing_path: clamp(location.pathname, 500),
      source: clamp(source, 200),
      referrer_host: clamp(referrerHost, 255) || null,
  ...
  function controlCode(el) {
    ...
    if (el.tagName === 'SELECT') {
      return `${name}#${Math.max(0, el.selectedIndex) + 1}`;
    }
    return name;
  }
  ...
  document.addEventListener('click', e => {
    ...
    else if (el.matches('button,a')) event('cta_click', { value_code: `control:${normalize(el.textContent || el.getAttribute('aria-label')).toLowerCase()}` });
  }, true);
  ...
  document.addEventListener('submit', e => {
    ...
    if (email && String(email.value || '').trim()) {
      event('email_provided', { metadata: { method: 'form_submit' } });
    }
  }, true);
  ...
  function maybeCompletion() {
    const forms = document.querySelectorAll('form');
    const text = document.body.innerText.toLowerCase();
    const completedSignal = /complete|finished|results are ready|your score/.test(text);
    if (!completed && completedSignal && answerCount > 0) {
      completed = true;
      event('completion', { metadata: { answer_count: answerCount } });
    }
  }
  setInterval(() => {
